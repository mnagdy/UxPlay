...
/* A stopped preparation cannot resume from delayed FCUP responses. */
void airplay_video_cancel_pending(airplay_video_t *airplay_video);
bool airplay_video_is_cancelled(const airplay_video_t *airplay_video);
...
/* Sanitized counters from the last profile filter; no sender content. */
const char *airplay_video_get_cache_diagnostics(const airplay_video_t *airplay_video);
