...
static void test_clock_survives_repeated_initialization(void) {
    /* Hold an independent user of the same singleton, like the video renderer.
     * Audio construction/destruction must neither steal its reference nor
     * accumulate new ones across successive AirPlay sessions. */
    GstClock *shared_clock = gst_system_clock_obtain();
    gint references = GST_OBJECT_REFCOUNT_VALUE(shared_clock);
    bool sync_enabled = false;
    for (int i = 0; i < 16; ++i) {
        audio_renderer_init(logger, "fakesink", &sync_enabled, &sync_enabled, "");
        audio_renderer_destroy();
        g_assert_cmpint(GST_OBJECT_REFCOUNT_VALUE(shared_clock), ==, references);
        g_assert_cmpuint(gst_clock_get_time(shared_clock), !=, GST_CLOCK_TIME_NONE);
    }
...
static void test_startup_failure_allows_same_codec_retry(void) {
    install_test_renderers();
    audio_renderer_t *owner = renderer_type[0];
    GstElement *sink = gst_bin_get_by_name(GST_BIN(owner->pipeline), "test_sink");
    g_object_set(sink, "state-error", 3, NULL);
    unsigned char ct = 8;
    /* Fail PAUSED -> PLAYING as a sink/driver can at startup. A failed
     * selection must not remain active and suppress a same-codec retry. */
    audio_renderer_start(&ct);
    g_assert_null(renderer);
    g_assert_false(render_audio);
    GstState state;
...
int main(int argc, char **argv) {
    ...
    g_test_add_func("/audio-renderer/repeated-init-shared-clock", test_clock_survives_repeated_initialization);
    ...
    g_test_add_func("/audio-renderer/startup-failure-same-codec-retry", test_startup_failure_allows_same_codec_retry);
