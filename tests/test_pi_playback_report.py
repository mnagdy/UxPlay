...
class DiagnosticTests(unittest.TestCase):
    ...
    def test_wayland_fields_still_reject_unknown_values(self):
        for field in ("vo", "decode_policy"):
            with self.subTest(field=field):
                record, reason = report.parse_diagnostic(
                    f"MPV playback: session=1 state=playing {field}=PRIVATE_SENTINEL")
                self.assertIsNone(record)
                self.assertEqual(reason, "invalid_known_record")
                self.assertNotIn("PRIVATE_SENTINEL", reason)
...
class RetainedTests(unittest.TestCase):
    ...
    def test_wayland_trial_fields_survive_retained_export(self):
        expected = [
            {"session": index, "state": "playing", "vo": output,
             "decode_policy": "pi4-wayland-experimental"}
            for index, output in enumerate(("dmabuf-wayland", "wlshm"), 1)
        ]
        messages = [entry("MPV playback: " + " ".join(f"{key}={value}" for key, value in fields.items()))
                    for fields in expected]
        self.write_file("playback-trace.jsonl", b"".join(json.dumps(row).encode() + b"\n" for row in messages))
        rows, stats = self.collect()
        self.assertEqual(stats["omitted_by_reason"], {})
        encoded = report.encode_report({"schema": report.SCHEMA, "journal": {}, "records": rows}, 8000)
        self.assertCountEqual([row["fields"] for row in json.loads(encoded)["records"]], expected)
        self.assertNotIn("PRIVATE_SENTINEL", encoded.decode())
