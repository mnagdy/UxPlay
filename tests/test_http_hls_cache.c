...
typedef struct {
    ...
    unsigned int play_calls, feedback_calls;
...
static void sender_feedback(void *cls) { ((fixture_t *)cls)->feedback_calls++; }
...
static void fixture_init(fixture_t *f) {
    ...
    f->raop.callbacks.conn_feedback = sender_feedback;
    f->conn.raop = &f->raop;
...
static void expect_request(const char *expected_url) {
    ...
    if (expected_url) {
        ...
        /* XML escapes signed URL queries; compare the decoded URI exactly. */
        char *body = strstr(received, "\r\n\r\n");
        assert(body);
        body += 4;
        plist_t root = NULL;
        plist_from_xml(body, strlen(body), &root);
        assert(PLIST_IS_DICT(root));
        plist_t request = plist_dict_get_item(root, "request");
        plist_t url = plist_dict_get_item(request, "FCUP_Response_URL");
        assert(PLIST_IS_STRING(url));
        char *actual_url = NULL;
        plist_get_string_val(url, &actual_url);
        assert(actual_url && !strcmp(actual_url, expected_url));
        plist_mem_free(actual_url);
        plist_free(root);
    } else {
        assert(length == 0);
    }
...
static void test_master_query_identity(bool scoped) {
    static const char requested_master[] =
        "mlhls://fixture.invalid/current-video/master.m3u8?token=one%2Ftwo&itag=100";
    static const char wrong_master[] =
        "mlhls://fixture.invalid/current-video/master.m3u8?token=other&itag=100";
    fixture_t f;
    fixture_init(&f);
    f.raop.hls_scoped_cache = scoped;
    unsigned char local[4] = {127, 0, 0, 1};
    f.conn.remote = local;
    f.conn.remotelen = sizeof(local);
    plist_t root = plist_new_dict();
    plist_dict_set_item(root, "uuid", plist_new_string(playback_uuid));
    plist_dict_set_item(root, "Content-Location", plist_new_string(requested_master));
    plist_dict_set_item(root, "clientProcName", plist_new_string("YouTube"));
    assert(invoke(&f, http_handler_play, "/play", root) == 200);
    expect_request(requested_master);
    airplay_video_t *video = hls_get_current_video(&f.raop);
    assert(video && !airplay_video_is_ready(video));
    /* A missing/changed query is a different response even with the right ID. */
    assert(action_response(&f, master_url, 1, 200, master) == 200);
    assert(action_response(&f, wrong_master, 1, 200, master) == 200);
    expect_request(NULL);
    assert(get_num_media_uri(video) == 0);
    assert(action_response(&f, requested_master, 1, 200, master) == 200);
    ...
    expect_request(NULL);
    assert(f.play_calls == 1 && f.request_error_calls == 0);
    assert(airplay_video_is_ready(video));
    const char *local_path = f.played_location + strlen("http://localhost:7000");
    assert(strstr(local_path, "/master.m3u8?token=one%2Ftwo&itag=100"));
    char reply[8192], changed_path[256];
    assert(invoke_plain(&f, http_handler_hls, "GET", local_path, NULL, reply, sizeof(reply)) == 200);
    assert(strstr(reply, "#EXTM3U"));
    snprintf(changed_path, sizeof(changed_path), "%s", local_path);
    *strchr(changed_path, '?') = '\0';
    assert(invoke_plain(&f, http_handler_hls, "GET", changed_path, NULL, reply, sizeof(reply)) == 404);
    strcat(changed_path, "?token=other&itag=100");
    assert(invoke_plain(&f, http_handler_hls, "GET", changed_path, NULL, reply, sizeof(reply)) == 404);
    fixture_destroy(&f);
}

static void test_mpv_external_rendition_collection(bool external_audio) {
    static const char subtitle_uri[] = "https://fixture.invalid/subtitles.m3u8?token=PRIVATE%2Fsubtitle&return=mlhls://fixture.invalid/current-video/original";
    static const char audio_uri[] = "https://fixture.invalid/audio.m3u8?token=PRIVATE%2Faudio&return=mlhls://fixture.invalid/current-video/original";
    char playlist[2048];
    snprintf(playlist, sizeof(playlist),
        "#EXTM3U\n"
        "#EXT-X-MEDIA:TYPE=AUDIO,GROUP-ID=\"audio\",NAME=\"English\",LANGUAGE=\"en\",DEFAULT=YES,URI=\"%s\"\n"
        "#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID=\"subs\",NAME=\"English\",LANGUAGE=\"en\",URI=\"%s\"\n"
        "#EXT-X-STREAM-INF:BANDWIDTH=100,CODECS=\"avc1.4d401f,mp4a.40.2\",RESOLUTION=1280x720,AUDIO=\"audio\",SUBTITLES=\"subs\"\n%s\n",
        external_audio ? audio_uri : media0, subtitle_uri, media1);
    fixture_t f;
    fixture_init(&f);
    ...
    assert(action_response(&f, master_url, 1, 200, playlist) == 200);
    airplay_video_t *video = hls_get_current_video(&f.raop);
    assert(video && get_num_media_uri(video) == (external_audio ? 1 : 2));
    unsigned request_id = 2;
    if (!external_audio) {
        expect_request(media0);
        assert(action_response(&f, media0, request_id++, 200, media) == 200);
    }
    expect_request(media1);
    assert(f.play_calls == 0 && f.request_error_calls == 0);
    assert(action_response(&f, media1, request_id, 200, media) == 200);
    expect_request(NULL); /* Direct HTTP(S) renditions never enter the FCUP queue. */
    assert(f.play_calls == 1 && f.request_error_calls == 0);
    assert(airplay_video_is_ready(video));
    const char *diagnostics = airplay_video_get_cache_diagnostics(video);
    assert(!strstr(diagnostics, "PRIVATE") && !strstr(diagnostics, "://"));
    char reply[8192];
    const char *path = f.played_location + strlen("http://localhost:7000");
    assert(invoke_plain(&f, http_handler_hls, "GET", path, NULL, reply, sizeof(reply)) == 200);
    assert(strstr(reply, subtitle_uri));
    if (external_audio) assert(strstr(reply, audio_uri));
    fixture_destroy(&f);
}
...
static void test_invalid_master_paths_do_not_replace_current_request(void) {
    static const char *locations[] = {
        "/master.m3u8",
        "mlhls://fixture.invalid/current-video/master.m3u8-extra",
        "mlhls://fixture.invalid/current-video/other?redirect=/master.m3u8",
        "mlhls://fixture.invalid/current-video/other#/master.m3u8",
        "mlhls://fixture.invalid/current-video/master.m3u8/other"
    };
    fixture_t f;
    fixture_init(&f);
    f.raop.hls_scoped_cache = true;
    airplay_video_t *current = start_collection(&f);
    for (unsigned i = 0; i < sizeof(locations) / sizeof(locations[0]); i++) {
        ...
        assert(!airplay_video_is_cancelled(current));
        assert(f.request_calls == 1 && f.request_error_calls == 0 && f.reset_calls == 0);
        expect_request(NULL);
    }
    fixture_destroy(&f);
}
...
static void test_missing_or_malformed_master_is_rejected(void) {
    const char *bodies[] = {"not found", "not a playlist", "#EXTM3U\n#EXT-X-VERSION:3\n"};
    for (unsigned i = 0; i < sizeof(bodies)/sizeof(bodies[0]); i++) {
        fixture_t f;
        fixture_init(&f);
        play_request(&f);
        expect_request(master_url);
        assert(action_response(&f, master_url, 1, i ? 200 : 404, bodies[i]) == 400);
        expect_request(NULL);
        assert(f.play_calls == 0);
        airplay_video_t *video = hls_get_current_video(&f.raop);
        assert(get_num_media_uri(video) == 0);
        assert(get_next_media_uri_id(video) == 0);
        assert(!airplay_video_is_ready(video));
        assert(airplay_video_is_cancelled(video));
        fixture_destroy(&f);
    }
}
...
static plist_t setup_key_request(size_t iv_length, size_t key_length) {
    const char bytes[73] = {0};
    assert(iv_length <= sizeof(bytes) && key_length <= sizeof(bytes));
    plist_t root = plist_new_dict();
    plist_dict_set_item(root, "eiv", plist_new_data(bytes, iv_length));
    plist_dict_set_item(root, "ekey", plist_new_data(bytes, key_length));
    return root;
}

static void test_setup_key_bounds(void) {
    fixture_t f;
    fixture_init(&f);
    logger_set_level(f.raop.logger, LOGGER_DEBUG);
    const size_t malformed[][2] = {{1, 72}, {0, 72}, {15, 72}, {17, 72},
                                    {16, 0}, {16, 1}, {16, 71}, {16, 73}};
    for (size_t i = 0; i < sizeof(malformed) / sizeof(malformed[0]); i++) {
        assert(invoke_request(&f, raop_handler_setup, "SETUP", "RTSP/1.0", "/fixture",
                              setup_key_request(malformed[i][0], malformed[i][1])) == 400);
        assert(!f.conn.raop_ntp && !f.conn.raop_rtp && !f.conn.raop_rtp_mirror);
    }
    plist_t missing = setup_key_request(16, 72);
    plist_dict_remove_item(missing, "eiv");
    assert(invoke_request(&f, raop_handler_setup, "SETUP", "RTSP/1.0", "/fixture", missing) == 400);
    plist_t mistyped = setup_key_request(16, 72);
    plist_dict_set_item(mistyped, "ekey", plist_new_string("invalid"));
    assert(invoke_request(&f, raop_handler_setup, "SETUP", "RTSP/1.0", "/fixture", mistyped) == 400);
    assert(invoke_request(&f, raop_handler_setup, "SETUP", "RTSP/1.0", "/fixture", plist_new_array()) == 400);
    assert(!f.conn.raop_ntp && !f.conn.raop_rtp && !f.conn.raop_rtp_mirror);
    fixture_destroy(&f);
}
...
static void test_audio_teardown_lifecycle(void) {
    ...
    /* SETUP 1 must not overwrite a live transport owner with new objects.
     * Valid later SETUP 2 / typed TEARDOWN continue on these same objects. */
    raop_ntp_t *original_ntp = f.conn.raop_ntp;
    for (int retry = 0; retry < 3; retry++) {
        assert(invoke_request(&f, raop_handler_setup, "SETUP", "RTSP/1.0", "/fixture",
                              setup_key_request(16, 72)) == 455);
        assert(f.conn.raop_ntp == original_ntp && f.conn.raop_rtp == original_rtp);
        assert(raop_rtp_is_running(original_rtp));
...
static void test_scoped_controls_reject_stale_session(void) {
    ...
    assert(f.feedback_calls == 1); /* stale and missing-session polls do not count */
    for (unsigned i = 0; i < 20; ++i)
        assert(invoke_plain(&f, http_handler_playback_info, "GET", "/playback-info", session_id, NULL, 0) == 200);
    assert(f.feedback_calls == 21);
...
static void test_early_request_error_callback(void) {
    ...
    assert(action_response(&f, master_url, 1, 200, master) == 200);
    expect_request(NULL);
    assert(f.play_calls == 0 && f.request_error_calls == 1);
...
static void count_flush(void *cls) { ++*(unsigned *)cls; }

static void test_nonmedia_connection_does_not_flush_video(void) {
    unsigned flushes = 0;
    raop_t owner = {0};
    owner.logger = logger_init();
    logger_set_level(owner.logger, LOGGER_ERR);
    owner.callbacks.cls = &flushes;
    owner.callbacks.video_flush = count_flush;
    const connection_type_t types[] = {CONNECTION_TYPE_UNKNOWN, CONNECTION_TYPE_AIRPLAY,
                                      CONNECTION_TYPE_PTTH, CONNECTION_TYPE_HLS};
    for (unsigned i = 0; i < sizeof(types)/sizeof(types[0]); i++) {
        raop_conn_t *conn = calloc(1, sizeof(*conn));
        assert(conn);
        conn->raop = &owner;
        conn->connection_type = types[i];
        conn_destroy(conn);
    }
    assert(flushes == 0);
    logger_destroy(owner.logger);
}

static void test_stop_cancels_delayed_playlist_reply(void) {
    fixture_t f;
    fixture_init(&f);
    f.raop.hls_scoped_cache = true;
    airplay_video_t *video = start_collection(&f);
    assert(invoke_plain(&f, http_handler_stop, "POST", "/stop", session_id, NULL, 0) == 200);
    assert(airplay_video_is_cancelled(video));
    assert(action_response(&f, media0, 2, 200, media) == 200);
    expect_request(NULL);
    assert(f.play_calls == 0 && f.request_error_calls == 0);
    /* Retrying on the same sender session starts a fresh acquisition. */
    play_request(&f);
    expect_request(master_url);
    assert(!airplay_video_is_cancelled(hls_get_current_video(&f.raop)));
    fixture_destroy(&f);
}

static void test_invalid_controls_and_borrowed_property_nodes(void) {
    fixture_t f;
    fixture_init(&f);
    f.raop.hls_scoped_cache = true;
    f.raop.hls_support = true;
    assert(invoke_plain(&f, http_handler_set_property, "PUT", "/setProperty?selectedMediaArray",
                        session_id, NULL, 0) == 409);
    start_collection(&f);
    const char *queries[] = {"?position", "?position=", "?position=nan", "?position=inf",
                            "?position=-1", "?position=2junk", "?position=1e99", "?other=4"};
    for (unsigned i = 0; i < sizeof(queries)/sizeof(queries[0]); i++) {
        char url[100];
        snprintf(url, sizeof(url), "/scrub%s", queries[i]);
        assert(invoke_plain(&f, http_handler_scrub, "POST", url, session_id, NULL, 0) == 400);
    }
    assert(f.scrub_calls == 0);
    assert(invoke_plain(&f, http_handler_rate, "POST", "/rate?value", session_id, NULL, 0) == 400);
    assert(invoke_plain(&f, http_handler_rate, "POST", "/rate?value=nan", session_id, NULL, 0) == 400);
    assert(f.rate_calls == 0);
    assert(invoke_plain(&f, http_handler_scrub, "POST", "/scrub?position=2.5", session_id, NULL, 0) == 200);
    const char *stale = "99999999-2222-3333-4444-555555555555";
    assert(invoke_plain(&f, NULL, "PUT", "/setProperty?selectedMediaArray", stale, NULL, 0) == 409);
    assert(f.conn.connection_type == CONNECTION_TYPE_UNKNOWN);
    plist_t root = plist_new_dict(), array = plist_new_array();
    /* An incomplete entry is owned by the array, never freed independently. */
    plist_array_append_item(array, plist_new_dict());
    plist_t selection = plist_new_dict();
    plist_dict_set_item(selection, "MediaSelectionOptionsName", plist_new_string("English"));
    plist_dict_set_item(selection, "MediaSelectionOptionsUnicodeLanguageIdentifier", plist_new_string("en"));
    plist_array_append_item(array, selection);
    plist_dict_set_item(root, "value", array);
    assert(invoke_request(&f, http_handler_set_property, "PUT", "HTTP/1.1", "/setProperty?selectedMediaArray", root) == 200);
    assert(!strcmp(get_language_code(hls_get_current_video(&f.raop)), "en"));
    root = plist_new_dict(); array = plist_new_array(); selection = plist_new_dict();
    plist_dict_set_item(selection, "MediaSelectionOptionsName", plist_new_string(""));
    plist_dict_set_item(selection, "MediaSelectionOptionsUnicodeLanguageIdentifier", plist_new_string(""));
    plist_array_append_item(array, selection);
    plist_dict_set_item(root, "value", array);
    assert(invoke_request(&f, http_handler_set_property, "PUT", "HTTP/1.1", "/setProperty?selectedMediaArray", root) == 200);
    assert(!strcmp(get_language_code(hls_get_current_video(&f.raop)), "en"));
...
int main(void) {
    test_mpv_external_rendition_collection(false);
    test_mpv_external_rendition_collection(true);
    test_setup_key_bounds();
    test_master_query_identity(false);
    test_master_query_identity(true);
    test_invalid_master_paths_do_not_replace_current_request();
    test_nonmedia_connection_does_not_flush_video();
    test_stop_cancels_delayed_playlist_reply();
    test_invalid_controls_and_borrowed_property_nodes();
