...
static guint mirror_progress_logs;
static gchar mirror_progress_log[1024];
...
static void capture_log(void *cls, int level, const char *text) {
    ...
    if (g_str_has_prefix(text, "Mirror progress: ")) {
        mirror_progress_logs++;
        g_strlcpy(mirror_progress_log, text, sizeof(mirror_progress_log));
    }
}
...
static void test_bounded_mirror_progress_logging(void) {
    screen_status_init(SCREEN_INFO_STATUS, "PRIVATE_SCREEN_FIXTURE");
    video_renderer_configure_screen(SCREEN_INFO_STATUS);
    video_renderer_t owner = {0};
    g_mutex_init(&owner.overlay_lock);
    owner.pipeline = gst_pipeline_new(NULL);
    owner.codec = h264;
    owner.screen_generation = screen_status_begin_session(SCREEN_SESSION_MIRRORING, SCREEN_ROUTE_RTP,
                                                         "PRIVATE_SCREEN_FIXTURE");
    owner.diagnostics = playback_diagnostics_attach(owner.pipeline, test_logger, 0, owner.screen_generation);
    hls_video = false;
    renderer = &owner;
    mirror_progress_logs = 0;
    /* Exercise the actual status-mode refresh, without a debug overlay or
     * attached pad probes. Unavailable counters must not look like zero. */
    video_renderer_screen_refresh();
    g_assert_cmpuint(mirror_progress_logs, ==, 1);
    g_assert_nonnull(strstr(mirror_progress_log, "input_known=0 input_buffers=-1"));
    g_assert_nonnull(strstr(mirror_progress_log, "decoded_known=0 decoded_buffers=-1"));
    g_assert_nonnull(strstr(mirror_progress_log, "output_known=0 output_buffers=-1"));
    g_assert_nonnull(strstr(mirror_progress_log, "input_age_known=0 input_age_ms=-1"));
    g_assert_nonnull(strstr(mirror_progress_log, "output_age_known=0 output_age_ms=-1"));
    g_assert_nonnull(strstr(mirror_progress_log, "state_result=1 state_known=1 current=1 pending=0"));
    screen_status_snapshot_t screen;
    screen_status_get_snapshot(&screen);
    playback_diagnostics_snapshot_t observed = {0};
    observed.video.input_buffers_known = observed.video.decoded_buffers_known = observed.video.output_buffers_known = TRUE;
    observed.video.input_buffers = 9;
    observed.video.decoded_buffers = 1;
    observed.video.output_buffers = 0; /* Observed zero is different from unknown. */
    gint64 started = owner.mirror_progress_started_us;
    observed.last_media_at_us = started + G_USEC_PER_SEC;
    observed.last_output_at_us = started;
    video_renderer_log_mirror_progress(&owner, &screen, &observed, started + 2 * G_USEC_PER_SEC - 1);
    g_assert_cmpuint(mirror_progress_logs, ==, 1);
    video_renderer_log_mirror_progress(&owner, &screen, &observed, started + 2 * G_USEC_PER_SEC);
    g_assert_cmpuint(mirror_progress_logs, ==, 2);
    g_assert_nonnull(strstr(mirror_progress_log, "input_known=1 input_buffers=9"));
    g_assert_nonnull(strstr(mirror_progress_log, "decoded_known=1 decoded_buffers=1"));
    g_assert_nonnull(strstr(mirror_progress_log, "output_known=1 output_buffers=0"));
    g_assert_nonnull(strstr(mirror_progress_log, "input_age_known=1 input_age_ms=1000"));
    g_assert_nonnull(strstr(mirror_progress_log, "output_age_known=1 output_age_ms=2000"));
    for (guint step = 4; step <= 80; step += 2)
        video_renderer_log_mirror_progress(&owner, &screen, &observed, started + step * G_USEC_PER_SEC);
    g_assert_cmpuint(mirror_progress_logs, ==, 16);
    g_assert_cmpuint(owner.mirror_progress_reports, ==, 16);
    g_assert_nonnull(strstr(mirror_progress_log, "report=16 elapsed_ms=30000"));
    /* A delayed timer cannot restart or extend the thirty-second window. */
    owner.mirror_progress_reports = 1;
    owner.mirror_progress_last_us = started;
    video_renderer_log_mirror_progress(&owner, &screen, &observed, started + 31 * G_USEC_PER_SEC);
    g_assert_cmpuint(mirror_progress_logs, ==, 16);
    /* Neither cover art, HLS, ended sessions nor replaced generations qualify. */
    owner.mirror_progress_reports = 0;
    owner.codec = jpeg;
    video_renderer_log_mirror_progress(&owner, &screen, &observed, started);
    owner.codec = h264;
    hls_video = true;
    video_renderer_log_mirror_progress(&owner, &screen, &observed, started);
    hls_video = false;
    screen.session_active = false;
    video_renderer_log_mirror_progress(&owner, &screen, &observed, started);
    screen.session_active = true;
    screen.generation++;
    video_renderer_log_mirror_progress(&owner, &screen, &observed, started);
    g_assert_cmpuint(mirror_progress_logs, ==, 16);
    g_assert_cmpuint(owner.mirror_progress_reports, ==, 0);
    renderer = NULL;
    playback_diagnostics_free(owner.diagnostics);
    gst_object_unref(owner.pipeline);
    clear_owner(&owner);
    video_renderer_configure_screen(SCREEN_INFO_OFF);
}
...
int main(int argc, char **argv) {
    ...
    test_bounded_mirror_progress_logging();
