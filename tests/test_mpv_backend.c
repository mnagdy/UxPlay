...
static void test_pi_wayland_mode(void)
{
    char error[200];
    mpv_backend_config_t c = config();
    c.decode_policy = MPV_DECODE_PI4_WAYLAND_EXPERIMENTAL;
    c.qualified_h264_hwdec = "v4l2m2m";
    c.video_output = "dmabuf-wayland";
    setenv("UXPLAY_FAKE_MPV_MODE", "normal", 1);
    mpv_backend_t *b = mpv_backend_create(&c, error, sizeof(error));
    assert(b);
    assert(mpv_backend_open(b, 1, "http://fixture/wayland", 0));
    await_state(b, MPV_BACKEND_PLAYING);
    cleanup(b);

    const char *bad_outputs[] = {NULL, "", "gpu", "dmabuf-wayland,gpu"};
    for (size_t i = 0; i < sizeof(bad_outputs) / sizeof(bad_outputs[0]); ++i) {
        c.video_output = bad_outputs[i];
        assert(!mpv_backend_create(&c, error, sizeof(error)));
        assert(strstr(error, "Pi Wayland requires"));
    }
    c.video_output = "dmabuf-wayland";
    const char *bad_decoders[] = {NULL, "", "auto", "v4l2m2m-copy"};
    for (size_t i = 0; i < sizeof(bad_decoders) / sizeof(bad_decoders[0]); ++i) {
        c.qualified_h264_hwdec = bad_decoders[i];
        assert(!mpv_backend_create(&c, error, sizeof(error)));
        assert(strstr(error, "Pi Wayland requires"));
    }
    c.qualified_h264_hwdec = "v4l2m2m";
    const char **conflicts[] = {&c.gpu_context, &c.gpu_api, &c.drm_device, &c.drm_connector};
    const char *values[] = {"drm", "opengl", "/dev/dri/card1", "HDMI-A-1"};
    for (size_t i = 0; i < sizeof(conflicts) / sizeof(conflicts[0]); ++i) {
        *conflicts[i] = values[i];
        assert(!mpv_backend_create(&c, error, sizeof(error)));
        assert(strstr(error, "no GPU/DRM options"));
        *conflicts[i] = ""; /* Empty optional values are equivalent to absent. */
    }
    b = mpv_backend_create(&c, error, sizeof(error));
    assert(b && mpv_backend_destroy(b));
}
...
static void test_retiring_ipc_failure(const char *mode)
{
    mpv_backend_t *b = create(mode);
    assert(mpv_backend_open(b, 1, "https://fixture/retiring", 0));
    await_state(b, MPV_BACKEND_PLAYING);
    int old_pid = snapshot(b).child_pid;
    setenv("UXPLAY_FAKE_MPV_MODE", "normal", 1);
    assert(mpv_backend_open(b, 2, "https://fixture/replacement", 0));
    assert(mpv_backend_pause(b, 2));
    await_state(b, MPV_BACKEND_PAUSED);
    mpv_backend_snapshot_t current = snapshot(b), retired;
    assert(current.generation == 2 && current.child_generation == 2);
    assert(current.child_pid != old_pid && current.error_code == MPV_BACKEND_ERROR_NONE);
    assert(current.ready && current.requested_paused);
    assert(mpv_backend_take_terminal_snapshot(b, &retired));
    assert(retired.generation == 1 && retired.child_pid == old_pid && !retired.child_alive);
    assert(retired.error_code == MPV_BACKEND_ERROR_IPC && retired.state == MPV_BACKEND_FAILED);
    assert(!strcmp(retired.failure_stage, "ipc"));
    assert(mpv_backend_resume(b, 2));
    await_state(b, MPV_BACKEND_PLAYING);
    cleanup(b);
}

static void test_startup_without_playback_restart(bool paused)
{
    mpv_backend_t *b = create("no-playback-restart");
    assert(mpv_backend_open(b, 1, "https://fixture/output-startup-stall", 0));
    if (paused) assert(mpv_backend_pause(b, 1));
    await_state(b, paused ? MPV_BACKEND_PAUSED : MPV_BACKEND_PLAYING);
    assert(snapshot(b).ready && !snapshot(b).playback_restarted);
    await_state(b, MPV_BACKEND_FAILED);
    assert(snapshot(b).error_code == MPV_BACKEND_ERROR_LOAD);
    assert(strstr(snapshot(b).error, "timed out"));
    setenv("UXPLAY_FAKE_MPV_MODE", "normal", 1);
    assert(mpv_backend_open(b, 2, "https://fixture/recovered", 0));
    await_state(b, MPV_BACKEND_PLAYING);
    assert(snapshot(b).generation == 2 && !snapshot(b).error_code);
    cleanup(b);
}
...
int main(int argc, char **argv)
{
    ...
    test_retiring_ipc_failure("stop-malformed");
    test_retiring_ipc_failure("stop-invalid");
    test_retiring_ipc_failure("stop-oversized");
    test_startup_without_playback_restart(false);
    test_startup_without_playback_restart(true);
    test_seek_failure();
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strstr(line, "\"argv\"")) {
            ...
            if (strstr(line, "--hwdec=drm,v4l2m2m")) {
                ...
                if (strstr(line, "--vo=dmabuf-wayland")) {
                    wayland_mode = true;
                    assert(!strstr(line, "--gpu-context="));
                    assert(!strstr(line, "--gpu-api="));
                    assert(!strstr(line, "--gpu-hwdec-interop="));
                    assert(!strstr(line, "--drm-"));
                } else {
                    hevc_mode = true;
                    assert(strstr(line, "--vo=gpu"));
                    assert(strstr(line, "--gpu-context=drm"));
                    assert(strstr(line, "--gpu-api=opengl"));
    ...
    assert(fast_rendering && default_rendering && hevc_mode && wayland_mode && initial_start);
    unlink(log_path);
    unlink(lock_path);
    close(extra_pipe[0]);
    close(extra_pipe[1]);
