...
## Live SSH debug switch

After installing this build and `scripts/tv-debug` on the receiver user's PATH:
...
`off` selects normal status messages; `on` selects the detailed overlay. Commands
apply to the existing stream without restarting UxPlay or mpv. mpv updates within
about half a second; GStreamer updates on the next video frame after its display
refresh, so a paused GStreamer picture retains its old text until frames resume.
A successful reply confirms the mode was accepted and saved, not HDMI visibility.

The receiver owns a private Unix datagram socket in `~/.uxplay-control` (0700).
Only the same account can use it; sudo is unnecessary. Requests are bounded and
handled in the main loop. A lock prevents another receiver from replacing the
endpoint. The saved `mode` overrides the initial `screen-info` display selection
on subsequent starts and survives release changes. Remove that file while the
receiver is stopped to return to configuration-only selection. Failure to save
leaves the active setting unchanged; an absent/older receiver produces an error.
The helper never falls back to restarting playback.

The live switch changes presentation only. Packet capture and diagnostic-history
policy retain their startup configuration; enabling the overlay does not start
extra packet capture midstream. GStreamer keeps its observation hooks available
when starting with screen information off so a later live switch can work.

A new receiver binary requires one initial service restart. Normal CMake install
includes `tv-debug`; Pi development releases also contain the helper for manual
installation into `~/.local/bin` or `/usr/local/bin`.

Validation for the live switch (11 September 2026): the Pi release
`20260911T142247396217Z-52c2997e3800-dirty` compiled successfully. Pi CTest suites
`receiver_backends`, `screen_video`, `screen_status`, and `screen_status_renderer`
passed. The receiver fixture sends real local socket requests while its synthetic
mpv child plays and pauses, checks unchanged child/session/capture identity,
repeated commands, invalid requests, persistence, and save-failure behaviour.
The 24 Python helper/deployment tests passed locally. These are headless checks;
real HDMI picture, sound, and GStreamer paused-frame behaviour await activation.
The current service was not restarted because unattended sudo is unavailable.
`~/enable-tv-debug` on this Pi points to the prepared release's activation helper;
it uses the existing deployment health checks and rollback handling.
