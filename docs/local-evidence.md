...
## Continuous display trial — 11 September

[Milestone findings](continuous-display.md) and the local evidence directory
`.pi-dev/continuous-display-live-20260911/` record the first real DRM compositor
trial. Weston and the receiver survived two mpv `dmabuf-wayland` initialization
crashes. The diagnostic GPU Wayland alternative worked for 720p but dropped 570
output frames in the bounded 4K60 Main 10 attempt. Receiver termination restored
the original service, which then played and stopped the same 720p fixture.
These observations do not close physical picture, sound, or no-flash acceptance.

The [12 September follow-up](../.pi-dev/continuous-display-live-20260912/README.md)
records the corrected player startup and the user's acceptance of HEVC 720p25,
H.264 1080p60 and 4K60 Main 10 picture/sound at 720p60 HDMI. Terminal flashes
were absent on the first two start/stop cycles; Main 10 returned cleanly to
ready. Its full-session drop counters were unavailable after EOF. Portrait
iPhone mirroring failed on a fullscreen geometry error; the normal service
was restored. `r2-acceptance.json` preserves the tested boundaries and pending
mirroring/handover/recovery checks.

R3 then displayed the first phone frame and froze. Source review and a native
GStreamer timing reproduction identified a retry that could apply the sender
clock offset twice, scheduling the first frame far into the future. The R4
correction passed native receiver/screen-video tests and preflight; its live
start initially required the Pi sudo password. After reboot R4 passed physical
mirroring, responsive rotation, no-flash disconnect and YouTube picture/sound.
Compositor termination restored the normal service. See
`.pi-dev/mirror-timing-result.json`, the R4 acceptance record and the maintained
milestone page. The R5 audio follow-up below records the final trial boundary.
...
## Audio-only follow-up, 12 September

The R4/R5 observations and logs are indexed in the
[continuous-display trial record](../.pi-dev/continuous-display-live-20260912/README.md).
R4 exposed a feedback-watchdog disconnect despite arriving ALAC audio; R5
refreshes liveness from valid delivered ALAC. Its callback/audio tests passed,
and the user confirmed the connection stayed up while startup choppiness and
intermittent pauses remained. A fresh session with Wi-Fi power saving off then
passed steady startup audio and a clean no-flash disconnect. Retransmissions
persisted, so the network setting is not a proven cause. This is a single
passing audio run, not long-run reliability. The trial ended with the baseline
service restored and Wi-Fi power saving verified on; final evidence is in `r5/`.

## Durable display service — 13 September

[Startup integration](display-service.md) is now installed as the enabled Pi
receiver service. `.pi-dev/display-service-20260913/` retains the manifests,
unit verification, root/noexec reproduction, test transcripts and live recovery
records. V2 failed because `/run` forbids executable mappings; rollback worked.
V3 moved private code to the service cache and passed native/root preflight,
activation, receiver-only and whole-stack recovery, abrupt supervisor failure,
installed rollback/reactivation and automatic startup after reboot. The user
confirmed ready after startup/recovery/reboot. Postboot mirroring and a fresh
YouTube video passed picture/sound/ready return. An earlier YouTube E3 remains
unresolved, and repeated Spotify audio is choppy or stops while phone progress
continues. The numeric capture showed a 9.401-second incoming-audio gap on
5 GHz with zero capture drops; the stronger 2.4 GHz comparison also failed.
Original Wi-Fi profile restoration and trial-profile deletion were verified.
The user deferred this investigation; maintained issue notes are in
[playback findings](playback-findings.md#deferred-during-piplay-display-integration--13-september),
and `status.json` records the latest acceptance boundary.
