...
Consolidated 10 September and updated 13 September 2026 from retained investigations and test
...
### Deferred during PiPlay display integration — 13 September

At the user's request, pause these investigations and continue the display
milestone. They remain known playback limitations, not resolved defects or
blockers to further display work. No further audio/network experiments are
scheduled.

- **Spotify audio-only AirPlay chops or loses sound while phone progress
  continues.** Repeated starts failed with Wi-Fi power saving off. A 90-second
  capture on 5 GHz observed a 9.401-second gap in incoming audio packets with
  zero capture drops. Many retransmissions were duplicates; their count is not
  a packet-loss percentage. A temporary stronger 2.4 GHz access-point comparison
  also failed the user's listening test. Earlier receiver logs show every
  delivered frame accepted by appsrc, which does not establish audible output.
  Sender/network delivery and receiver recovery behavior remain unresolved;
  repeated resend requests may contribute but are not a confirmed cause.
- **Intermittent YouTube E3: “Stream data unavailable.”** One postboot request
  failed during playlist preparation about 60 ms after acceptance, before mpv
  started. A different video subsequently played with sound and a clean return
  to ready. The failing playlist field/stage is not logged, so the cause remains
  unknown. The error recurred at 07:16:16 on 13 September (session 13),
  about 983 ms after acceptance. This attempt received a master playlist and
  selected its default English audio from 21 languages, then failed before
  the player started. This narrows the observed stage to playlist preparation;
  it does not establish a codec, language-selection, or source defect. The
  display service remained active with no restarts. Preserve
  `youtube-e3-recurrence.log` for a later focused investigation. For immediate
  use, iPhone Screen Mirroring takes a separate path from direct HLS loading.

The temporary Wi-Fi comparison has ended. The original NetworkManager profile
was restored and the trial profile deleted; it automatically selected a 2.4 GHz
access point on restoration. Wi-Fi power saving remains off under the existing
display-service policy. The receiver stayed active with no service restarts.
Capture processes have ended; no receiver buffering or resend policy was changed.

Startup, reboot, receiver/compositor recovery, responsive phone mirroring and
accepted video-to-ready transitions remain verified separately. Keep these
playback issues separate from startup/loading animation work. The service is
not being claimed fully reliable for all media.

Evidence: `.pi-dev/display-service-20260913/` contains both numeric header
captures, receiver journals, the 5 GHz summary and `status.json`. If revisited,
start with these records rather than repeating the completed tests.
...
**Channel 4: FairPlay Streaming incompatibility confirmed on 11 September.**
The real diagnostic attempt returned HTTP 200 playlists declaring SAMPLE-AES,
`com.apple.streamingkeydelivery` and an `skd` key URI. mpv logged key-open failure
before AAC/H.264 decoder errors. Software decoding also stalled at position zero,
so changing Pi hardware decoding or buffers cannot resolve this key-delivery path.
UxPlay/mpv does not implement the required FairPlay Streaming client. This is a
confirmed cause, not restored playback; a supported protected-playback endpoint
is needed. See the [captured Channel 4 findings](diagnostics/2026-09-11-channel4.md#confirmed-cause-unsupported-fairplay-streaming-key-delivery).

The earlier GStreamer 1.26.2 AES-128 key-download bug remains independently
reproduced and covered by the [isolated validation patch](../patches/README.md).
That patch rejects failed/malformed keys cleanly; it does not implement FairPlay
or solve the now-confirmed Channel 4 incompatibility.
