...
typedef enum {
    ...
    MPV_DECODE_PI4_WAYLAND_EXPERIMENTAL
} mpv_decode_policy_t;
...
typedef struct {
     ...
     * path; the experimental HEVC and Wayland policies require v4l2m2m.
     * WAYLAND also requires dmabuf-wayland output and no GPU/DRM options.
     * Selecting a policy does not establish qualification on the device. */
