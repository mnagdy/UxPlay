                                  ...
                                  char *error, size_t error_size)
{
    ...
    if (config->decode_policy != MPV_DECODE_SOFTWARE &&
        ...
        config->decode_policy != MPV_DECODE_PI4_WAYLAND_EXPERIMENTAL) {
        copy_text(error, error_size, "Invalid mpv decoding policy");
        return NULL;
    }
    if (config->decode_policy == MPV_DECODE_PI4_WAYLAND_EXPERIMENTAL &&
        (!config->qualified_h264_hwdec || strcmp(config->qualified_h264_hwdec, "v4l2m2m") ||
         !config->video_output || strcmp(config->video_output, "dmabuf-wayland") ||
         (config->gpu_context && *config->gpu_context) ||
         (config->gpu_api && *config->gpu_api) ||
         (config->drm_device && *config->drm_device) ||
         (config->drm_connector && *config->drm_connector))) {
        copy_text(error, error_size, "mpv Pi Wayland requires dmabuf-wayland, H.264 v4l2m2m and no GPU/DRM options");
        return NULL;
    }
...
static void fail(mpv_backend_t *b, const char *message)
{
    ...
    if (b->pid && b->child_generation != b->snapshot.generation) {
        /* Invalid bytes still belong to their socket's retiring child. An
         * error during replacement must not cancel the new URL waiting for
         * that child to exit. Keep the diagnosis on the old report and let
         * the existing quit/TERM/KILL deadlines finish releasing its output. */
        if (b->retiring_valid && b->retiring.generation == b->child_generation) {
            if (b->retiring.error_code == MPV_BACKEND_ERROR_NONE) {
                b->retiring.error_code = MPV_BACKEND_ERROR_IPC;
                copy_text(b->retiring.failure_stage, sizeof(b->retiring.failure_stage), "ipc");
                copy_text(b->retiring.error, sizeof(b->retiring.error), message);
            }
            b->retiring.state = MPV_BACKEND_FAILED;
            b->retiring.ready = false;
        }
        close_ipc(b);
        return;
    }
...
static bool spawn_player(mpv_backend_t *b)
{
    ...
    } else if (b->decode_policy == MPV_DECODE_PI4_WAYLAND_EXPERIMENTAL) {
        /* Submit decoded buffers to the compositor. It owns HDMI and plane
         * assignment; mpv must not acquire a DRM display or use DRM overlays.
         * Fail closed if hardware decoding cannot satisfy this experiment. */
        argv[argc++] = "--hwdec=drm,v4l2m2m";
        argv[argc++] = "--hwdec-codecs=h264,hevc";
        argv[argc++] = "--hwdec-software-fallback=no";
    } else {
...
void mpv_backend_poll(mpv_backend_t *b)
{
    ...
    if (b->pid) {
        ...
        if (!b->stop_requested && !b->stop_step && b->want_session) {
            ...
            /* file-loaded permits controls, but output initialization can
             * still stall before the first playback-restart. Keep the load
             * deadline through that boundary for zero-position starts too. */
            else if (!b->preflight && b->load_at &&
                     (!b->snapshot.ready || !b->snapshot.playback_restarted || b->load_start_pending) &&
                     now - b->load_at >= b->load_ms)
