...
struct video_renderer_s {
    ...
    gint64 mirror_progress_started_us, mirror_progress_last_us;
    guint mirror_progress_reports;
#ifdef  X_DISPLAY_FIX
    bool use_x11;
        ...
        if (hls_video) {
            ...
            if (!rtp || jpeg_pipeline) {
                ...
                if (video_sync && !jpeg_pipeline) {
                    ...
                    renderer_type[i]->sync = true;
                } else {
                    g_string_append(launch, " sync=false");
                    renderer_type[i]->sync = false;
                }
            }
...
static int video_renderer_cycle_unlocked(void) {
    ...
    for (int i = 0 ; i < 2; i++) {
        /* Keep the previous one-second state wait, but make it a deadline
         * rather than repeating it forever when new coverart never arrives.
         * This bounds our polling, not a plugin/driver blocked in set_state. */
        ...
        while (state != target_state) {
            ...
            if (remaining <= 0) {
                logger_log(logger, LOGGER_WARNING, "pipeline %s: state change to %s timed out",
                           renderer->codec, gst_element_state_get_name(target_state));
                return -1;
            }
            ret = gst_element_get_state(renderer->pipeline, &state, &pending_state,
                                        (GstClockTime)remaining * GST_USECOND);
            if (ret == GST_STATE_CHANGE_SUCCESS) {
...
static int video_renderer_choose_codec_unlocked(bool video_is_jpeg, bool video_is_h265) {
    ...
    if (gst_element_get_state(renderer->pipeline, &old_state, &new_state, 100 * GST_MSECOND) == GST_STATE_CHANGE_FAILURE) {
        /* A decoder/output failure belongs to this session. g_error aborts
         * the whole receiver before video_set_codec can handle our result. */
...
int video_renderer_cycle(void) {
    g_rec_mutex_lock(&direct_mutex);
    int result = video_renderer_cycle_unlocked();
    g_rec_mutex_unlock(&direct_mutex);
    return result;
}
...
bool video_get_playback_info_with_readiness(double *duration, double *position, double *seek_start,
    ...
    /* A constructed playbin is only a shell until this request has started
     * and preroll has completed. Buffering may pause a ready VOD pipeline;
     * report that separately from its ability to keep up with playback. */
    bool ready = renderer && (!hls_video ||
        (!direct_state.failed && renderer->direct_generation == direct_generation &&
         renderer->direct_started && (direct_state.preroll_complete || direct_state.live)));
    bool likely = ready && (!hls_video || !direct_state.buffering ||
                            direct_state.live || renderer->gap_only_audio);
    if (ready_to_play) *ready_to_play = ready;
    if (likely_to_keep_up) *likely_to_keep_up = likely;
    g_rec_mutex_unlock(&direct_mutex);
    return result;
}
...
static gint64 mirror_progress_age_ms(gint64 observed_at_us, gint64 now_us) {
    return observed_at_us > 0 && now_us >= observed_at_us ? (now_us - observed_at_us) / 1000 : -1;
}

static gint64 mirror_progress_count(gboolean known, guint64 count) {
    return known ? (gint64) MIN(count, (guint64) G_MAXINT64) : -1;
}

static void video_renderer_log_mirror_progress(video_renderer_t *owner,
    const screen_status_snapshot_t *screen, const playback_diagnostics_snapshot_t *observed, gint64 now_us) {
    if (hls_video || !owner->pipeline || !owner->codec ||
        (strcmp(owner->codec, h264) && strcmp(owner->codec, h265)) ||
        screen->kind != SCREEN_SESSION_MIRRORING || !screen->session_active ||
        screen->generation != owner->screen_generation || !owner->screen_generation ||
        owner->mirror_progress_reports >= 16) return;
    if (!owner->mirror_progress_reports) owner->mirror_progress_started_us = now_us;
    gint64 elapsed_us = now_us - owner->mirror_progress_started_us;
    if (elapsed_us < 0 || elapsed_us > 30 * G_USEC_PER_SEC ||
        (owner->mirror_progress_reports && now_us - owner->mirror_progress_last_us < 2 * G_USEC_PER_SEC)) return;
    GstState current = GST_STATE_VOID_PENDING, pending = GST_STATE_VOID_PENDING;
    GstStateChangeReturn result = gst_element_get_state(owner->pipeline, &current, &pending, 0);
    gboolean state_known = result != GST_STATE_CHANGE_FAILURE;
    const screen_status_video_t *video = &observed->video;
    gint64 input_age = mirror_progress_age_ms(observed->last_media_at_us, now_us);
    gint64 output_age = mirror_progress_age_ms(observed->last_output_at_us, now_us);
    owner->mirror_progress_last_us = now_us;
    owner->mirror_progress_reports++;
    /* Counts are decoder-input, decoded-video and video-sink buffers. The
     * snapshot is allowlisted; do not add caps, sender text or clock epochs.
     * Unknown observations remain -1, distinct from a measured zero. */
    logger_log(logger, LOGGER_INFO,
        "Mirror progress: session=%" G_GUINT64_FORMAT " report=%u elapsed_ms=%" G_GINT64_FORMAT
        " input_known=%u input_buffers=%" G_GINT64_FORMAT
        " decoded_known=%u decoded_buffers=%" G_GINT64_FORMAT
        " output_known=%u output_buffers=%" G_GINT64_FORMAT
        " dropped_known=%u dropped_frames=%" G_GINT64_FORMAT
        " input_age_known=%u input_age_ms=%" G_GINT64_FORMAT
        " output_age_known=%u output_age_ms=%" G_GINT64_FORMAT
        " state_result=%u state_known=%u current=%d pending=%d failed=%u",
        owner->screen_generation, owner->mirror_progress_reports, elapsed_us / 1000,
        (guint) video->input_buffers_known, mirror_progress_count(video->input_buffers_known, video->input_buffers),
        (guint) video->decoded_buffers_known, mirror_progress_count(video->decoded_buffers_known, video->decoded_buffers),
        (guint) video->output_buffers_known, mirror_progress_count(video->output_buffers_known, video->output_buffers),
        (guint) video->dropped_known, mirror_progress_count(video->dropped_known, video->dropped_frames),
        (guint) (input_age >= 0), input_age, (guint) (output_age >= 0), output_age,
        (guint) result, (guint) state_known, state_known ? (gint) current : -1,
        state_known ? (gint) pending : -1, (guint) observed->failed);
}
...
void video_renderer_screen_refresh(void) {
    ...
    video_renderer_log_mirror_progress(owner, &snapshot, &observed, g_get_monotonic_time());
