...
static bool create_locked(screen_status_renderer_t *r, const char *text, screen_info_mode_t mode) {
    ...
    /* A persistent Wayland background needs prompt loading/error feedback.
     * Direct DRM keeps its existing idle cost and release behavior. */
    int refresh_rate = !strcmp(r->sink_factory, "waylandsink") ? 10 : 1;
    GstCaps *caps = gst_caps_new_simple("video/x-raw", "width", G_TYPE_INT, 1280,
        "height", G_TYPE_INT, 720, "framerate", GST_TYPE_FRACTION, refresh_rate, 1, NULL);
