...
static bool check_startup = false;
static screen_info_mode_t screen_info = SCREEN_INFO_OFF;
/* Opt-in: a separately supervised Wayland compositor owns HDMI. The status
 * surface stays mapped below each transient media window. */
static bool compositor_owns_display = false;
static std::atomic<bool> compositor_display_failed(false);
...
static const char *mpv_policy_name() {
    switch (mpv_policy) {
    case MPV_DECODE_SOFTWARE: return "software";
    case MPV_DECODE_PI4_SAFE: return "pi4-safe";
    case MPV_DECODE_PI4_HEVC_EXPERIMENTAL: return "pi4-hevc-experimental";
    case MPV_DECODE_PI4_WAYLAND_EXPERIMENTAL: return "pi4-wayland-experimental";
    }
    return "unknown";
}

/* Both startup-only validation and normal startup use the same effective
 * configuration and the backend's real argv/IPC handshake. No media is loaded. */
static bool check_mpv_startup(char *error, size_t error_size) {
    if (!use_video || videosink == "0") {
        g_strlcpy(error, "mpv AirPlay video requires video output enabled", error_size);
        return false;
    }
    ...
    mpv_config.disable_audio = !use_audio || audiosink == "0";
    mpv_config.packet_diagnostics = screen_info == SCREEN_INFO_DEBUG;
    return mpv_backend_preflight(&mpv_config, error, error_size);
}
#endif
...
static gboolean feedback_callback(gpointer loop) {
    if (open_connections) {
        const guint missed = missed_feedback.load(std::memory_order_relaxed);
        if (missed_feedback_limit && missed > missed_feedback_limit) {
            ...
            LOGE("%3u seconds since last client feedback request (expected every two seconds); client may be offline", missed);
        }
        missed_feedback++;
    } else {
        missed_feedback = 0;
...
#ifdef UXPLAY_HAVE_MPV
    if (direct_video && airplay_video_mpv) policy = mpv_policy_name();
#endif
    g_strlcpy(video.decode_policy, policy, sizeof(video.decode_policy));
    screen_status_set_video(current.generation, &video);
}

static bool persistent_status_display_available() {
    if (compositor_display_failed) return false;
    /* Updating the existing surface does not raise or remap it. Only final
     * shutdown may destroy this background in compositor mode. */
    if (status_display && screen_status_renderer_tick(status_display) &&
        screen_status_renderer_is_visible(status_display)) return true;
    compositor_display_failed = true;
    relaunch_video = false;
    screen_status_event(screen_generation(), SCREEN_EVENT_RECOVERY_REQUIRED);
    LOGE("Persistent display lost; stopping receiver for supervised recovery");
    return false;
}

static bool hide_status_display() {
    if (compositor_owns_display) return persistent_status_display_available();
...
/* Validate before any sink can open. Mixed Wayland/direct-DRM configuration
 * defeats persistent ownership, so it is rejected instead of falling back. */
static const char *display_owner_config_error(bool require_live_environment = true) {
#ifdef UXPLAY_HAVE_MPV
    if (!compositor_owns_display && airplay_video_mpv &&
        mpv_policy == MPV_DECODE_PI4_WAYLAND_EXPERIMENTAL)
        return "pi4-wayland-experimental requires display-owner wayland";
#endif
    if (!compositor_owns_display) return NULL;
    if (!use_video || videosink != "waylandsink" || !rtp_pipeline.empty() ||
        videosink_options.find('!') != std::string::npos)
        return "display-owner wayland requires a plain waylandsink video output";
    if (videosink_options.find("fullscreen") != std::string::npos)
        return "display-owner wayland sets fullscreen automatically; remove fullscreen sink properties";
    const char *wayland_display = getenv("WAYLAND_DISPLAY");
    const char *runtime_dir = getenv("XDG_RUNTIME_DIR");
    if (require_live_environment &&
        (!wayland_display || !*wayland_display || !runtime_dir || !*runtime_dir))
        return "display-owner wayland requires a running compositor and its Wayland environment";
#ifdef UXPLAY_HAVE_MPV
    if (airplay_video_mpv) {
        const bool native_wayland = mpv_vo == "dmabuf-wayland" || mpv_vo == "wlshm";
        const bool gpu_wayland = (mpv_vo == "gpu" || mpv_vo == "gpu-next") &&
            (mpv_context == "wayland" || mpv_context == "waylandvk");
        if ((!native_wayland && !gpu_wayland) || !mpv_drm_device.empty() || !mpv_connector.empty() ||
            (native_wayland && (!mpv_context.empty() || !mpv_api.empty())) ||
            mpv_policy == MPV_DECODE_PI4_HEVC_EXPERIMENTAL)
            return "display-owner wayland requires an explicit Wayland mpv output without direct DRM options";
    }
#endif
    return NULL;
}
...
static void show_status_display() {
    if (compositor_owns_display && compositor_display_failed) return;
...
static void format_mpv_trace(const mpv_backend_snapshot_t &p, char *text, size_t capacity, bool terminal_report = false) {
    ...
    g_snprintf(text + used, capacity - used,
        ...
        mpv_trace_label(p.video_output, "|gpu|gpu-next|drm|dmabuf-wayland|wlshm|x11|xv|null|"),
...
static gboolean receiver_screen_tick(gpointer loop) {
    std::lock_guard<std::mutex> guard(display_mutex);
    /* Check before any early return in media preparation. A missing
     * background must stop the receiver even while a player is only queued;
     * cleanup then stops/reaps any child through its existing supervisor. */
    if (compositor_owns_display && !persistent_status_display_available()) {
        relaunch_video = false;
    ...
    if (mpv_player) {
        ...
        if (mpv_owns_output && player.generation == mpv_generation && !mpv_rebuild_pending) {
            ...
            const char *policy = mpv_policy_name();
            g_strlcpy(video.decode_policy, policy, sizeof(video.decode_policy));
            ...
            if (screen_status_get_mode() != SCREEN_INFO_OFF && player.child_alive &&
    ...
    if (now - last_refresh >= G_USEC_PER_SEC) {
        ...
        if (status_display && !compositor_owns_display) screen_status_renderer_tick(status_display);
        last_refresh = now;
    }
    return TRUE;
}
...
static void main_loop()  {
    ...
    missed_feedback = 0;
    guint screen_watch_id = g_timeout_add(25, receiver_screen_tick, loop);
    guint debug_watch_id = g_timeout_add(100, debug_control_tick, NULL);
    ...
    g_source_remove(debug_watch_id);
...
static void print_info (char *name) {
    ...
    printf("-check-startup  Check configured mpv options and IPC, then exit without outputs\n");
    printf("-screen-info off|status|debug  HDMI receiver status (default off)\n");
    printf("-display-owner renderer|wayland  Keep a background under media in Wayland mode\n");
    printf("-mpv-decode software|pi4-safe|pi4-hevc-experimental  Default software\n");
    printf("          pi4-wayland-experimental uses compositor DMA-BUF output (unqualified)\n");
...
static void parse_arguments (int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-check-startup")) check_startup = true;
    }
    for (int i = 1; i < argc; i++) {
        if (!is_utf8(argv[i], NULL)) {
            ...
            exit(check_startup ? 1 : 0);
        }
    }
    for (int i = 1; i < argc; i++) {
        std::string arg(argv[i]);
        if (arg == "-check-startup") {
            check_startup = true;
        } else if (arg == "-rc") {
	    ...
	    if (i <  argc - 1) {
            ...
            if (check_startup) {
                fprintf(stderr, "Startup check cannot be combined with help or version options\n");
                exit(1);
            }
            print_info(argv[0]);
            exit(0);
	  ...
	  audio_rtp_pipeline.append(argv[++i]);
	    ...
	    int n;
            ...
            i++;
        } else if (arg == "-airplay-video-backend" || arg == "-screen-info" ||
                   arg == "-display-owner" || arg.compare(0, 5, "-mpv-") == 0) {
            if (i + 1 >= argc || argv[i + 1][0] == '-') {
                fprintf(stderr, "%s requires a value\n", arg.c_str()); exit(1);
            }
            std::string value = argv[++i];
            if (arg == "-display-owner") {
                if (value != "renderer" && value != "wayland") {
                    fprintf(stderr, "display-owner must be renderer or wayland\n"); exit(1);
                }
                compositor_owns_display = value == "wayland";
            } else if (arg == "-screen-info") {
                ...
                if (arg == "-mpv-decode") {
                    ...
                    else if (value == "pi4-wayland-experimental") mpv_policy = MPV_DECODE_PI4_WAYLAND_EXPERIMENTAL;
...
static int parse_dmap_header(const unsigned char *metadata, char *tag, int *len) {
    ...
    uint32_t length = 0;
    for (int i = 0; i < 4; i++) {
        length = (length << 8) | *header;
        header++;
    }
    *len = 0;
    if (!istag || length > INT_MAX) {
        return 1;
    }
    *len = (int) length;
    return 0;
...
extern "C" void video_reset(void *cls, reset_type_t type) {
    ...
    if ((status_display || airplay_video_mpv) && type != RESET_TYPE_ON_VIDEO_PLAY)
...
extern "C" void audio_process (void *cls, raop_ntp_t *ntp, audio_decode_struct *data) {
    ...
    /* Legacy audio-only ALAC senders need not send /feedback while streaming.
     * Decrypted audio delivered by the active RTP receiver proves liveness;
     * empty/format-only packets never reach this callback. Keep the timeout
     * enabled so a sender that actually disappears is still reclaimed. */
    if (data->ct == 2 && data->data && data->data_len > 0 && data->data[0] == 0x20)
        conn_feedback(cls);
    if (dump_audio) {
...
#ifdef UXPLAY_RECEIVER_TEST
/* Test only: exercise the real timestamp/retry callback without a decoder or
 * physical output. Production calls the renderer directly. */
static auto mirror_render_buffer = video_renderer_render_buffer;
#else
#define mirror_render_buffer video_renderer_render_buffer
#endif
...
extern "C" void video_process (void *cls, raop_ntp_t *ntp, video_decode_struct *data) {
    ...
    if (use_video) {
        ...
        /* A retry must translate the original sender timestamp only once.
         * Applying the epoch offset to an already translated timestamp can
         * put the first frame years ahead of the pipeline's running time. */
        const uint64_t sender_timestamp = data->ntp_time_remote;
        int count = 0;
        uint64_t pts_mismatch = 0;
        uint64_t required_base_time = 0, total_correction = 0;
        do {
            data->ntp_time_remote = sender_timestamp + remote_clock_offset;
            pts_mismatch = mirror_render_buffer(data->data, &(data->data_len), &(data->nal_count), &(data->ntp_time_remote));
            if (pts_mismatch) {
                LOGI("adjust timestamps by %8.6f secs", (double) pts_mismatch / SECOND_IN_NSECS);
                /* The renderer reports exactly how far this timestamp is
                 * below its base time. Retain only relative diagnostic data. */
                required_base_time = data->ntp_time_remote + pts_mismatch;
                total_correction += pts_mismatch;
                remote_clock_offset += pts_mismatch;
            }
            count++;
        } while (pts_mismatch && count < 10);
        if (total_correction && !pts_mismatch) {
            LOGI("Mirror timing: correction_ms=%.3f corrected_frame_pts_ms=%.3f retries=%d",
                 (double) total_correction / (SECOND_IN_NSECS / 1000),
                 (double) (data->ntp_time_remote - required_base_time) / (SECOND_IN_NSECS / 1000),
                 count - 1);
        }
    }
}
...
extern "C" void audio_set_metadata(void *cls, const void *buffer, int buflen) {
    ...
    if (!buffer || buflen < 8) {
        LOGE("received invalid metadata buffer, length %d", buflen);
        return;
    ...
    while (buflen >= 8) {
        ...
        buflen -= 8;
        if (datalen > buflen) {
            LOGE("received truncated metadata item, length %d exceeds remaining %d", datalen, buflen);
            return;
        }
...
extern "C" void on_video_acquire_playback_info (void *cls, playback_info_t *playback_info) {
    ...
    if (mpv_player) {
        ...
        screen_status_snapshot_t screen;
        screen_status_get_snapshot(&screen);
        /* Preparation can fail or be stopped before a child gets this
         * generation. Report that terminal request before an older snapshot
         * can make the phone think the cancelled request is still buffering. */
        if (screen.kind == SCREEN_SESSION_DIRECT_VIDEO &&
            (screen.state == SCREEN_STATE_FAILED || screen.state == SCREEN_STATE_STOPPING ||
...
static bool read_config_file(const char * filename, const char * uxplay_name) {
...
int main (int argc, char *argv[]) {
    ...
    char *rcfile = NULL;
    /* Know the requested exit-only mode before opening the config, so a
     * missing explicit config cannot be mistaken for successful validation. */
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-check-startup")) check_startup = true;
    }
    ...
    if (config_file.length()) {
        if (!read_config_file(config_file.c_str(), argv[0]) && check_startup) exit(1);
    }
    parse_arguments (argc, argv);
...
#ifdef UXPLAY_HAVE_MPV
        if (!airplay_video_mpv) {
            LOGE("Startup check requires -airplay-video-backend mpv");
            exit(1);
        }
        /* An idle mpv opens no surface. Validate every display configuration
         * rule, but allow this check before the compositor is running. */
        if (const char *error = display_owner_config_error(false)) {
            LOGE("Display configuration: %s", error);
            exit(1);
        }
        char error[192];
        if (!check_mpv_startup(error, sizeof(error))) {
            LOGE("mpv startup check failed: %s", error);
            exit(1);
        }
        LOGI("Startup check passed: mpv options and private IPC; no media or outputs opened");
        exit(0);
#else
        LOGE("Startup check requires the mpv backend; configure with -DUXPLAY_ENABLE_MPV=ON");
        exit(1);
#endif
    }
    ...
    if (const char *error = display_owner_config_error()) {
        LOGE("Display configuration: %s", error);
        exit(1);
    }
    if (compositor_owns_display) {
        /* Keep the compositor's configured bounds when portrait or oversized
         * video arrives. The trial supplies waylandsink's upstream fix for
         * setting fullscreen before its first frame creates the window. */
        fullscreen = true;
        if (screen_info == SCREEN_INFO_OFF) screen_info = SCREEN_INFO_STATUS;
        LOGI("Display owner: Wayland compositor; retaining receiver background between media windows");
    }
    ...
    screen_info_mode_t initial_display_mode = screen_info;
    debug_control_init(&initial_display_mode);
    screen_status_init(initial_display_mode, server_name.c_str());
    screen_set_backend(true);
    video_renderer_configure_screen(initial_display_mode);
    if (use_video)
    ...
    if (use_video) {
        ...
        if (status_display || airplay_video_mpv) {
            ...
            show_status_display();
            if (compositor_owns_display && (!status_display ||
                !screen_status_renderer_is_visible(status_display))) {
                LOGE("Persistent Wayland background could not start"); exit(1);
            }
        }
#ifdef __OpenBSD__
    } else {
...
#define PID_MAX 4194304 // 2^22
    ...
    main_loop();
    if (relaunch_video && !compositor_display_failed) {
        if (reset_httpd) {
...
#endif
    exit(compositor_display_failed ? 1 : 0);
}
